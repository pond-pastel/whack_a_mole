...
let groundColorResetTimer = new Array(5).fill(null);
const GROUND_COLOR_RESET_TIME = 700;
let removeXmarkTimer = [];
let xmarkList = [];
let newMoleKey = [];
let score = 0;
let time = 0;
...
function groundColorReset(id) {
    const ground = document.querySelectorAll("#mole-group .ground");
    ground[id].style.background = null;
    clearTimeout(groundColorResetTimer[id]);
    groundColorResetTimer[id] = null;
}
...
function whack(k) {
    ...
    const moles = document.querySelectorAll("#mole-group .mole .mole-checkbox");
    const ground = document.querySelectorAll("#mole-group .ground");
    groundColorResetTimer[num] = setTimeout(groundColorReset, GROUND_COLOR_RESET_TIME, num);
    if(moles[num].checked) {
        score += 1;
        moles[num].checked = false;
        ground[num].style.background = "blue";
        moleAppears(num);
    } else {
        score -= 1;
        ground[num].style.background = "red";
    }
    scoreApply();
}
...
function addXmark(pos) {
    const xmark = document.createElement("div");
    xmark.className = "xmark";
    xmark.style.position = "fixed";
    xmark.style.left = `calc(${pos[0]}px - 0.5em)`;
    xmark.style.top = `calc(${pos[1]}px - 0.5em)`;
    const xmarkGroup = document.querySelector("#xmark-group");
    xmarkGroup.append(xmark);
    xmarkList.push(xmark);
    removeXmarkTimer.push(setTimeout(removeXmark, GROUND_COLOR_RESET_TIME));
}

function removeXmark() {
    xmarkList.shift().remove();
    clearTimeout(removeXmarkTimer.shift());
}
...
window.onload = function() {
    ...
    document.addEventListener("pointerdown", (e) => {
        touchMiss(e.target, [e.clientX, e.clientY]);
    });
    const moleGroup = document.querySelector("#mole-group");
    moleApply(moleGroup);
}